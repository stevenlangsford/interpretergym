const cloze_instructions =[
    ...
    "Type the next word into the text box in the middle of the left column. You need to type everything, not just the gaps.",
    "When you complete the target word, it will turn blue, and you move to the next word (or gap)",
    "Some notes: first, the meanings only <strong>mostly sort of</strong> correspond between the two columns. These are not necessarily good translations! Please take them as 'hints to help you guess' and not 'suggested correct answers'.",
    "You can/should ignore punctuation and case. Punctuation auto-completes.",
...
const cloze_motivation = "<h1>Why play Cross-language Cloze</h1>"+
      ...
      "<p>But what this does do is set up a guessing game a bit like a crossword puzzle, where the clues are both the local context and a meaning expressed in the other language.</p>"+
      "<p>Probably most of the value of this exercise is in getting comfortable 'shadowing' natural examples of the language you're inputting. The texts are supposed to be at least somewhat natural-sounding and packed full of interpreting-relevant terminology. But since the cross-language hints are there, there could also be some opportunities to disambiguate a cloze gap by exercising some kind translation-muscle occasionally?</p>"+
